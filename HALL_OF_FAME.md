# 🏆 AVE Database — Hall of Fame

[![Contributors](https://img.shields.io/badge/Contributors-5-blue)](./CONTRIBUTING.md)
[![Cards](https://img.shields.io/badge/AVE_Cards-50-green)](./ave-database/)

> **Recognising the security researchers who make agentic AI safer.**
>
> *Auto-generated on 2026-03-25 18:45 UTC from AVE Database card data.*

---

## 🎖️ Tier System

| Icon | Tier | XP Required | Description |
|------|------|-------------|-------------|
| ⭐ | **FELLOW** | 8,000+ XP | NAIL Research Fellow |
| 🏗️ | **ARCHITECT** | 4,000+ XP | Shaping the agentic safety taxonomy |
| 🛡️ | **SENTINEL** | 1,500+ XP | Trusted security researcher |
| 🏹 | **HUNTER** | 500+ XP | Proven vulnerability hunter |
| 👁️ | **WATCHER** | 0+ XP | Joined the hunt |

---

## 🏅 Leaderboard

| Rank | Tier | Contributor | XP | Cards | Badges | Streak |
|-----:|------|-------------|---:|------:|-------:|-------:|
| #1 | ⭐ fellow | **dleigh** | 10,650⚡ | 24 | ⭐🔭☠️📚🎯 +16 (21) | 3mo |
| #2 | 🏗️ architect | **s.nakamura** | 4,550⚡ | 8 | 💀🗺️🔥⚡🖐️ +6 (11) | 2mo |
| #3 | 🛡️ sentinel | **priya.r** | 3,250⚡ | 7 | 🗺️🔥🏰⚡🖐️ +7 (12) | 3mo |
| #4 | 🛡️ sentinel | **m.torres** | 2,975⚡ | 6 | 🗺️🔥🏰⚡🖐️ +8 (13) | 3mo |
| #5 | 🛡️ sentinel | **j.okafor** | 1,760⚡ | 5 | 🗺️🏰🖐️🕸️💡 +5 (10) | 3mo |

---

## 👤 Contributor Profiles

### ⭐ dleigh

**Tier:** FELLOW · **XP:** 10,650⚡ · **Cards:** 24 · **Rank:** #1

**Badges:** ⭐ NAIL Research Fellow 🔭 20/20 Vision ☠️ Doomsday Prophet 📚 Taxonomist 🎯 Ten Ring 💀 Critical Hunter 🎓 Polymath 🗺️ Category Pioneer 🔥 Trailblazer 🏰 Proved & Defended ⚡ Compound Threat 🖐️ Five Alive 🔴 Critical Finder 🐀 Lab Rat 🕸️ Web Weaver 💡 Novel Discovery 🎩 Hat Trick 🩸 First Blood 📊 Show Your Work 🛡️ Shield Bearer 🔗 Connector

**Cards:** `AVE-2025-0001`, `AVE-2025-0008`, `AVE-2025-0009`, `AVE-2025-0010`, `AVE-2025-0012`, `AVE-2025-0020`, `AVE-2025-0022`, `AVE-2025-0024`, `AVE-2025-0034`, `AVE-2025-0036`, `AVE-2025-0037`, `AVE-2025-0038`, `AVE-2025-0039`, `AVE-2025-0040`, `AVE-2025-0041`, `AVE-2025-0042`, `AVE-2025-0043`, `AVE-2025-0044`, `AVE-2025-0045`, `AVE-2025-0046`, `AVE-2025-0047`, `AVE-2025-0048`, `AVE-2025-0049`, `AVE-2025-0050`

---

### 🏗️ s.nakamura

**Tier:** ARCHITECT · **XP:** 4,550⚡ · **Cards:** 8 · **Rank:** #2

**Badges:** 💀 Critical Hunter 🗺️ Category Pioneer 🔥 Trailblazer ⚡ Compound Threat 🖐️ Five Alive 🔴 Critical Finder 🕸️ Web Weaver 💡 Novel Discovery 🩸 First Blood 📊 Show Your Work 🔗 Connector

**Cards:** `AVE-2025-0014`, `AVE-2025-0019`, `AVE-2025-0026`, `AVE-2025-0027`, `AVE-2025-0028`, `AVE-2025-0030`, `AVE-2025-0032`, `AVE-2025-0033`

**Progress to FELLOW:** `[███████████░░░░░░░░░]` 57% (3,450 XP needed)

---

### 🛡️ priya.r

**Tier:** SENTINEL · **XP:** 3,250⚡ · **Cards:** 7 · **Rank:** #3

**Badges:** 🗺️ Category Pioneer 🔥 Trailblazer 🏰 Proved & Defended ⚡ Compound Threat 🖐️ Five Alive 🕸️ Web Weaver 💡 Novel Discovery 🎩 Hat Trick 🩸 First Blood 📊 Show Your Work 🛡️ Shield Bearer 🔗 Connector

**Cards:** `AVE-2025-0002`, `AVE-2025-0005`, `AVE-2025-0007`, `AVE-2025-0015`, `AVE-2025-0021`, `AVE-2025-0023`, `AVE-2025-0025`

**Progress to ARCHITECT:** `[████████████████░░░░]` 81% (750 XP needed)

---

### 🛡️ m.torres

**Tier:** SENTINEL · **XP:** 2,975⚡ · **Cards:** 6 · **Rank:** #4

**Badges:** 🗺️ Category Pioneer 🔥 Trailblazer 🏰 Proved & Defended ⚡ Compound Threat 🖐️ Five Alive 🔴 Critical Finder 🕸️ Web Weaver 💡 Novel Discovery 🎩 Hat Trick 🩸 First Blood 📊 Show Your Work 🛡️ Shield Bearer 🔗 Connector

**Cards:** `AVE-2025-0003`, `AVE-2025-0011`, `AVE-2025-0013`, `AVE-2025-0017`, `AVE-2025-0029`, `AVE-2025-0035`

**Progress to ARCHITECT:** `[██████████████░░░░░░]` 74% (1,025 XP needed)

---

### 🛡️ j.okafor

**Tier:** SENTINEL · **XP:** 1,760⚡ · **Cards:** 5 · **Rank:** #5

**Badges:** 🗺️ Category Pioneer 🏰 Proved & Defended 🖐️ Five Alive 🕸️ Web Weaver 💡 Novel Discovery 🎩 Hat Trick 🩸 First Blood 📊 Show Your Work 🛡️ Shield Bearer 🔗 Connector

**Cards:** `AVE-2025-0004`, `AVE-2025-0006`, `AVE-2025-0016`, `AVE-2025-0018`, `AVE-2025-0031`

**Progress to ARCHITECT:** `[████████░░░░░░░░░░░░]` 44% (2,240 XP needed)

---

## 🏅 Badge Catalog

| Icon | Badge | Rarity | How to Earn |
|------|-------|--------|-------------|
| 🏆 | **Year of Living Dangerously** | ✨ legendary | 12 submissions in 12 consecutive months |
| ⭐ | **NAIL Research Fellow** | ✨ legendary | Achieved NAIL Research Fellow tier |
| 🔭 | **20/20 Vision** | 💎 epic | 20 accepted AVE cards |
| ☠️ | **Doomsday Prophet** | 💎 epic | Found 5+ CRITICAL severity vulnerabilities |
| 📚 | **Taxonomist** | 💎 epic | Submitted cards across 10+ different categories |
| 🎯 | **Ten Ring** | 💜 rare | 10 accepted AVE cards |
| 💀 | **Critical Hunter** | 💜 rare | Found 3+ CRITICAL severity vulnerabilities |
| ⚔️ | **Mitigation Master** | 💜 rare | 5+ cards with proven defences |
| 🗺️ | **Category Pioneer** | 💜 rare | First card in a previously empty category |
| 🎓 | **Polymath** | 💜 rare | Submitted cards across 5+ different categories |
| 🔥 | **Trailblazer** | 💜 rare | 5+ novel discoveries not previously documented |
| 📅 | **Half Year Hero** | 💜 rare | 6 submissions in 6 consecutive months |
| 🏰 | **Proved & Defended** | 💜 rare | A card that is both proven AND has mitigation |
| ⚡ | **Compound Threat** | 💜 rare | Card that references 3+ other AVEs |
| 🖐️ | **Five Alive** | 💚 uncommon | 5 accepted AVE cards |
| 🔴 | **Critical Finder** | 💚 uncommon | Found a CRITICAL severity vulnerability |
| 🐀 | **Lab Rat** | 💚 uncommon | 5+ cards with empirical evidence |
| 🧪 | **Proof Positive** | 💚 uncommon | Submitted a card with a working PoC |
| 🕸️ | **Web Weaver** | 💚 uncommon | 5+ cards with cross-references to other AVEs |
| 💡 | **Novel Discovery** | 💚 uncommon | Reported a previously undocumented vulnerability |
| 🎩 | **Hat Trick** | 💚 uncommon | 3 submissions in 3 consecutive months |
| 🩸 | **First Blood** | ⬜ common | Submitted your first AVE card |
| 📊 | **Show Your Work** | ⬜ common | Submitted a card with empirical evidence |
| 🛡️ | **Shield Bearer** | ⬜ common | Submitted a card with a known defence |
| 🔗 | **Connector** | ⬜ common | Linked your card to existing AVE cards |

---

## ⚡ How XP Works

### Base XP by Severity

| Severity | Base XP |
|----------|--------:|
| 🔴 critical | 500 XP |
| 🟠 high | 300 XP |
| 🟡 medium | 150 XP |
| 🟢 low | 75 XP |
| ℹ️ info | 50 XP |

### Status Multiplier

| Status | Multiplier |
|--------|----------:|
| `proven_mitigated` | ×1.2 |
| `proven` | ×1.0 |
| `theoretical` | ×0.5 |
| `in_progress` | ×0.3 |
| `not_proven` | ×0.1 |

### Bonus XP

| Action | Bonus |
|--------|------:|
| First card ever | +200 XP |
| Evidence with data | +50 XP (per item) |
| Proof of Concept | +100 XP |
| Defence/mitigation | +75 XP |
| Cross-references | +25 XP |
| Novel discovery | +150 XP |
| First in new category | +300 XP |

---

*Want to see your name here? [Submit a vulnerability](https://github.com/NAIL-INSTITUTE-FOR-AGENTIC-SECURITY/ave-database/issues/new?template=ave-submission.yml) and start earning XP!*

*Maintained by the [NAIL Institute](https://github.com/NAIL-INSTITUTE-FOR-AGENTIC-SECURITY/ave-database) — Building the safety infrastructure for agentic AI.*